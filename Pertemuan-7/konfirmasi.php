...
$dbname = "websiteberita"; // Ganti dengan nama database kamu
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $hobbies = isset($_POST['hobby']) ? implode(", ", $_POST['hobby']) : "None";
    $gender = htmlspecialchars($_POST['gender']);
    $tanggal_lahir = htmlspecialchars($_POST['dob']); // Mengganti 'dob' dengan 'tanggal_lahir'

    // Siapkan dan jalankan query untuk memasukkan data
    $sql = "INSERT INTO contacts (name, email, hobbies, gender, tanggal_lahir) VALUES ('$name', '$email', '$hobbies', '$gender', '$tanggal_lahir')"; // Mengganti 'dob' dengan 'tanggal_lahir'

    if ($conn->query($sql) === TRUE) {
        // Tampilkan pesan sukses
        echo "<h1>Kendala Anda sudah kami terima</h1>";
        echo "<p>Terima kasih sudah mengontak kami,</p>";
        echo "<p>Nama : <strong>$name</strong></p>";
        echo "<p>Email : <strong>$email</strong></p>";
        echo "<p>Hobby : <strong>$hobbies</strong></p>";
        echo "<p>Jenis Kelamin : <strong>$gender</strong></p>";
        echo "<p>Tanggal Lahir : <strong>$tanggal_lahir</strong></p>"; // Mengganti 'dob' dengan 'tanggal_lahir'
    } else {
